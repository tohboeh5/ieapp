...
describe("sqlSessionApi", () => {
  ...
  it("throws on create failure", async () => {
    server.use(
      http.post(
        testApiUrl("/spaces/sess-ws/sql-sessions"),
  ...
  it("throws on get failure", async () => {
    server.use(
      http.get(
        testApiUrl("/spaces/sess-ws/sql-sessions/bad"),
        () => HttpResponse.json({ detail: "Not found" }, { status: 404 }),
      ),
    );
    await expect(sqlSessionApi.get("sess-ws", "bad")).rejects.toThrow(
  ...
  it("throws on count failure", async () => {
    server.use(
      http.get(
        testApiUrl("/spaces/sess-ws/sql-sessions/bad/count"),
        () => HttpResponse.json({ detail: "Not found" }, { status: 404 }),
      ),
    );
  ...
  it("throws on rows failure", async () => {
    server.use(
      http.get(
        testApiUrl("/spaces/sess-ws/sql-sessions/bad/rows"),
        () => HttpResponse.json({ detail: "Not found" }, { status: 404 }),
      ),
    );
