...
describe("CreateFormDialog", () => {
  ...
  it("removes a column from create form dialog", async () => {
    ...
    expect(screen.queryByPlaceholderText("Column Name")).not
      .toBeInTheDocument();
  });
  ...
  it("REQ-FE-044: localizes create-form dialog copy in Japanese", async () => {
    ...
    expect(screen.getByRole("heading", { name: "新しいフォームを作成" }))
      .toBeInTheDocument();
    expect(screen.getByLabelText("名前")).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "+ カラムを追加" }))
      .toBeInTheDocument();
    expect(screen.getByRole("button", { name: "フォームを作成" }))
      .toBeInTheDocument();
  });
});
...
describe("CreateEntryDialog", () => {
  ...
  it("REQ-FE-068: pre-fills defaults for required fields", async () => {
    ...
    expect((startsAtInput as HTMLInputElement).value).toMatch(
  ...
  it("REQ-FE-053: renders English entry guidance across input modes", async () => {
    ...
    expect(
      screen.getByText(
        "Use true/false, yes/no, on/off, or 1/0 for boolean fields.",
      ),
    ).toBeInTheDocument();
    expect(
      screen.getByText(
  ...
  it("REQ-FE-065: row_reference fields offer a searchable picker while storing stable entry ids", async () => {
    ...
    await waitFor(() => {
      expect(searchApi.rowReferenceOptions).toHaveBeenCalledWith(
        "default",
        "Project",
        "alpha",
        8,
  ...
  it("REQ-FE-065: clearing a selected row_reference removes the saved entry id", async () => {
    ...
    expect(screen.queryByRole("button", { name: "Clear selection" })).not
      .toBeInTheDocument();
  });
  ...
  it("REQ-FE-065: row_reference fields without target_form keep raw text entry", async () => {
    const onSubmit = vi.fn();
    const onClose = vi.fn();
    const optionsSpy = vi.spyOn(searchApi, "rowReferenceOptions")
      .mockResolvedValue([]);
    const forms: Form[] = [
      {
        name: "Task",
  ...
  it("REQ-FE-057: chat step pills react to answers and current step", async () => {
    ...
    expect(screen.getByRole("button", { name: "Summary (required)" }))
      .toHaveClass(
        "ui-button-primary",
      );
    expect(screen.getByRole("button", { name: "Notes (optional)" }))
      .toHaveClass(
        "ui-button-secondary",
      );
    ...
    expect(screen.getByRole("button", { name: "Summary (answered)" }))
      .toHaveClass(
        "ui-button-primary",
      );
  ...
  it("REQ-FE-057: chat mode blocks advancing past an empty required field", async () => {
    ...
    expect(screen.getByText("Please answer required field: Summary."))
      .toBeInTheDocument();
    expect(screen.getByText("Question 1 / 2")).toBeInTheDocument();
    expect(onSubmit).not.toHaveBeenCalled();
  });
  ...
  it("REQ-FE-057: chat mode blocks skipping a required field", async () => {
    ...
    expect(screen.getByText("Required field cannot be skipped: Summary."))
      .toBeInTheDocument();
    expect(screen.getByText("Question 1 / 2")).toBeInTheDocument();
    expect(onSubmit).not.toHaveBeenCalled();
  });
...
describe("EditFormDialog", () => {
  ...
  it("REQ-FE-055: keeps edit-form column inputs readable on narrow layouts", async () => {
    ...
    if (!newInput) {
      throw new Error("Could not find new edit-dialog column input");
    }
